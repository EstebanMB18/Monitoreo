...
from src.fuentes.ecollect_parser import resumir_ecollect, archivo_publico_o_error
...
def procesar_archivos(corte='09'):
    ...
    for (codigo, tipo), cfg in eco_cfg.groupby(['codigo', 'tipo_reporte'], sort=False):
        ...
        if not cand:
            ...
            try:
                df = resumir_ecollect(cand[-1], medios_salida, cfg.iloc[0].vertical, codigo, 'ECOLLECT', tipo)
            except Exception as e:
