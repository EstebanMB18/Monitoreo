...
def finalize():
    cfg = load_config()
    root = output_root()
    deleted = cleanup_tree(PROJECT, root, cfg["retention"]["logs_days"], cfg["retention"]["downloads_days"], cfg["retention"]["max_log_files_per_folder"])
    dash = generate_dashboard(root)
